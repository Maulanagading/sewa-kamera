        ...
        <div class="card">
            <form action="{{ route('equipment.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="kategori" value="{{ old('kategori') }}" required placeholder="Contoh: Kamera DSLR, Lensa, Lighting">
                    @error('kategori') <span class="error">{{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                    <label>Harga Sewa Per Hari (Rp)</label>
                    <input type="number" name="harga_sewa_perhari" value="{{ old('harga_sewa_perhari') }}" required>
                ...
                <div class="form-group">
                    ...
                    <select name="status_ketersediaan" required>
                        ...
                        <option value="diperbaik" {{ old('status_ketersediaan') == 'diperbaik' ? 'selected' : '' }}>Diperbaiki</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea name="deskripsi_alat" rows="4">{{ old('deskripsi_alat') }}</textarea>
                    @error('deskripsi_alat') <span class="error">{{ $message }}</span> @enderror
                </div>

                <button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 10px;">Simpan Alat</button>
            </form>
        </div>
    </div>
</body>
